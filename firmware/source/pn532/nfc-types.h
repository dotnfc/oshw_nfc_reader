...
/**
 * @enum nfc_tag_type
 * @brief NFC tag type including d.e.p. type.
 */
typedef enum {
    TGT_NA,
    TGT_T1T,
 ...
 */
typedef struct {
  uint8_t  tg;                  /** Target ID for PN532 */

  nfc_dep_mode ndm;             /** DEP mode */
...
typedef enum {
  ...
  NMT_ISO14443A_TAG_EMU,        // 标签模拟
  NMT_ISO14443A_CARD_EMU,       // 卡模拟
  NMT_ISO14443A_P2P,            // 点对点
  NMT_ISO14443A_P2P_IN,         // 点对点: Initiator
  NMT_ISO14443A_P2P_TG,         // 点对点: Target
  NMT_MAX,          // size counter
} nfc_modulation_type;
 ...
 */
typedef struct _nfc_target_t {
    nfc_tag_type  ntt;          /** tag type */
    nfc_target_info nti;        /** target info */
    nfc_modulation nm;          /** target modulation */
} nfc_target_t;
typedef struct _nfc_target_t *nfc_target_p;
